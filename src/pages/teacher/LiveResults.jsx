...
export default function LiveResults() {
  ...
  // THÊM STATE VÀ HÀM ĐỔI FORMAT ĐIỂM
  const [scoreFormat, setScoreFormat] = useState('PERCENT'); // PERCENT, COUNT, OUT_OF_10
  const toggleScoreFormat = () => {
    if (scoreFormat === 'PERCENT') setScoreFormat('COUNT');
    else if (scoreFormat === 'COUNT') setScoreFormat('OUT_OF_10');
    else setScoreFormat('PERCENT');
  };
    ...
    <div style={{ padding: isMobile ? '15px' : '30px', minHeight: '100vh', backgroundColor: '#f8fafc', fontFamily: "'Josefin Sans', sans-serif" }}>
      ...
      {isSpaceRace ? (
        ...
        <>
          ...
          <div style={{ width: '100%', overflowX: 'auto', maxHeight: '70vh', overflowY: 'auto', backgroundColor: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
            <table style={{ minWidth: isMobile ? '600px' : '100%', borderCollapse: 'collapse', textAlign: 'center' }}>
              <thead style={{ position: 'sticky', top: 0, zIndex: 20 }}>
                <tr style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #cbd5e1' }}>
                  <th style={{ padding: '16px 20px', textAlign: 'left', color: '#475569', width: '240px', position: 'sticky', left: 0, top: 0, backgroundColor: '#f8fafc', zIndex: 30, boxShadow: 'inset 0 -1px 0 #cbd5e1' }}>Name</th>
                  <th onClick={toggleScoreFormat} style={{ padding: '16px', color: '#475569', width: '100px', cursor: 'pointer', userSelect: 'none', boxShadow: 'inset 0 -1px 0 #cbd5e1' }} title="Click để đổi định dạng">
                    Score ({scoreFormat === 'PERCENT' ? '%' : scoreFormat === 'COUNT' ? 'Câu' : '10'})
                  </th>
                  {quizData.questions.map((q, idx) => (
                    <th key={q.id} style={{ padding: '16px', color: (isTeacherPaced && currentQIndex === idx) ? '#0ea5e9' : '#003366', fontWeight: '800', borderBottom: (isTeacherPaced && currentQIndex === idx) ? '3px solid #0ea5e9' : 'none', boxShadow: 'inset 0 -1px 0 #cbd5e1' }}>
                      Q{idx + 1}
                    </th>
                  ))}
                </tr>
                ...
                {roster.length === 0 ? (
                  ...
                  roster.map((student, index) => {
                    ...
                    if (sub) {
                      ...
                      else if (scoreFormat === 'COUNT') scoreDisplay = `${correctCount}/${quizData.questions.length}`;
                      else scoreDisplay = ((correctCount / quizData.questions.length) * 10).toFixed(1);
                    }
