...
export default function RoomManager() {
  ...
  const handleAddRoom = async () => {
    ...
    if (rooms.some(r => r.roomId === roomNameUpper)) {
      alert("Tên phòng đã tồn tại!"); return;
    }
    try {
      const newRoom = { roomId: roomNameUpper, createdAt: new Date().toISOString(), students: [] };
  ...
  const handleDeleteRoom = async (roomId) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa phòng ${roomId}? Mọi dữ liệu sẽ bị mất.`)) {
      try {
  ...
  const handleAddStudent = async (e) => {
    ...
    } catch (error) { console.error("Lỗi thêm học viên:", error); }
  };
    ...
    <div style={{ position: 'relative', minHeight: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <div>
          ...
          <p style={{ color: '#64748b', marginTop: '5px' }}>Quản lý các lớp học ảo (delivery endpoints).</p>
        </div>
        <Button variant="success" style={{ width: 'auto', backgroundColor: '#f97316', padding: '12px 24px' }} onClick={() => setIsAddingRoom(true)}>
          + Add Room
        </Button>
      </div>
      ...
      {isAddingRoom && (
        <div style={{ padding: '20px', backgroundColor: 'white', border: '1px solid #e2e8f0', borderRadius: '12px', marginBottom: '20px', display: 'flex', gap: '15px', alignItems: 'flex-end', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
          <div style={{ flex: 1 }}><Input label="Room Name" placeholder="Nhập tên phòng..." value={newRoomName} onChange={(e) => setNewRoomName(e.target.value.toUpperCase())} /></div>
          <div style={{ marginBottom: '20px', display: 'flex', gap: '10px' }}>
            <Button variant="success" onClick={handleAddRoom} style={{ width: 'auto', padding: '12px 20px', backgroundColor: '#f97316' }}>Save</Button>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                <th style={{ padding: '15px 20px', color: '#475569', fontWeight: '700', textAlign: 'center', width: '80px' }}>In Menu</th>
                <th style={{ padding: '15px 20px', color: '#475569', fontWeight: '700', textAlign: 'center', width: '80px' }}>Status</th>
            ...
            <tbody>
              {rooms.length === 0 ? (
                <tr><td colSpan="6" style={{ padding: '30px', textAlign: 'center', color: '#94a3b8' }}>Chưa có phòng nào. Hãy tạo phòng mới!</td></tr>
              ) : (
                rooms.map((room, idx) => (
                  <tr key={room.id} style={{ borderBottom: '1px solid #f1f5f9', transition: 'background 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#f8fafc'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'white'}>
                    <td style={{ padding: '15px 20px', textAlign: 'center' }}>
                      <input type="checkbox" defaultChecked={idx === 0} style={{ transform: 'scale(1.2)', cursor: 'pointer' }} />
                    </td>
                    <td style={{ padding: '15px 20px', textAlign: 'center' }}>
                      <span style={{ display: 'inline-block', width: '12px', height: '12px', borderRadius: '50%', backgroundColor: idx === 0 ? '#10b981' : '#cbd5e1' }}></span>
                    </td>
                    <td style={{ padding: '15px 20px', color: '#003366', fontWeight: '700', fontSize: '18px' }}>
                      {room.roomId} <span style={{ cursor: 'pointer', fontSize: '14px', color: '#94a3b8', marginLeft: '8px' }}>✏️</span>
                      {idx === 0 && <span style={{ marginLeft: '15px', fontSize: '12px', backgroundColor: '#e0e7ff', color: '#1e40af', padding: '4px 8px', borderRadius: '4px', fontWeight: 'bold' }}>Default Room</span>}
                    </td>
                    <td style={{ padding: '15px 20px', textAlign: 'center' }}>
                      <button style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '18px' }} title="Chia sẻ">🔗</button>
                    </td>
                    <td style={{ padding: '15px 20px', textAlign: 'center' }}>
                      <button onClick={() => setRosterRoom(room)} style={{ background: 'none', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '6px 15px', cursor: 'pointer', color: '#003366', fontWeight: '600', display: 'inline-flex', alignItems: 'center', gap: '5px' }}>
                        👤 {room.students?.length || 0} ▾
                      </button>
                    </td>
                    <td style={{ padding: '15px 20px', textAlign: 'center' }}>
                      <button onClick={() => handleDeleteRoom(room.roomId)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '18px', color: '#94a3b8' }} title="Xóa phòng">🗑️</button>
                    </td>
                  </tr>
                ))
              )}
      ...
      {/* MODAL QUẢN LÝ ROSTER (Giữ nguyên logic cũ) */}
      {rosterRoom && (
