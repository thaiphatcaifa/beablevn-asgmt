  ...
  const handleLaunchActivity = async () => {
    ...
    if (activeMode === 'WEEKLY') {
      activeDaysToSave = weeklyDays.filter(d => d.quizId).map(d => {
        ...
        start.setHours(Number(oH), Number(oM), 0, 0);

        const [cH, cM] = d.closeTime.split(':');
        const end = new Date(baseDate); 
        end.setHours(Number(cH), Number(cM), 0, 0);
    ...
    try {
      ...
      if (activeMode === 'WEEKLY') {
         const now = Date.now();
         for (let i = 0; i < activeDaysToSave.length; i++) {
            if (now <= new Date(activeDaysToSave[i].endTime).getTime()) {
                initialDayIndex = i;
                break;
            }
         }
